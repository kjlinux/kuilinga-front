...
class FilterService {
  ...
  async getSites(organizationId: string): Promise<Site[]> {
    ...
    return response.items.filter(site => site.organization?.id === organizationId);
  }
  ...
  async getDepartments(siteId: string): Promise<Department[]> {
    ...
    return response.items.filter(dept => dept.site?.id === siteId);
  }
  ...
  async getEmployees(departmentId: string): Promise<Employee[]> {
    ...
    return response.items.filter(emp => emp.department?.id === departmentId);
  }
}
